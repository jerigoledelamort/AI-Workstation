...
# Check Open WebUI separately (optional)
try {
    $r = Invoke-RestMethod -Uri "http://127.0.0.1:8080/health" -Method GET -TimeoutSec 5
    Write-Host "[OK] Open WebUI" -ForegroundColor Green
} catch {
    Write-Host "[SKIP] Open WebUI (not started)" -ForegroundColor Yellow
}
